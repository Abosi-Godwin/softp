...
import type { ServiceKey, Step, TimeSlot, ContactInfo } from "./types";
...
export default function BookingsPage() {
    ...
    const { slots, loading: loadingSlots, markBooked } = useBookingSlots(step === "slot");
    ...
    const detailsValid = svc
        ? svc.fields.every(f => {
              ...
              return f.type === "chips" ? Array.isArray(v) && v.length > 0 : typeof v === "string" && v.trim().length > 0;
          })
        : false;
    ...
    const handleSubmit = async () => {
        ...
        try {
            await runTransaction(db, async tx => {
                ...
                if (slotDoc.exists()) throw new Error("This slot was just taken. Please choose another time.");
                tx.set(slotRef, {
                    service, answers, contact,
        ...
        <section className="min-h-screen bg-primary pt-32 pb-20 px-6 overflow-hidden">
            <div className="max-w-3xl mx-auto">
                ...
                <AnimatePresence mode="wait">
                    {step === "service" && (
                        <ServiceStep
                            onSelect={key => { setService(key); setStep("details"); }}
                        />
                    )}
